            ...
            if(!empty($apply_job)){
                ...
                if(!empty($device_token['devicetoken'])){
                  ...
                  if($device == 'android'){
                     ...
                     $notification_data['count']   = $notification_count;
                     $notification_data['user_id'] = $employer_id;
                     $notification_data['type']    = "applyjob";
                     send_android_notification($device_token['devicetoken'],$notification_data);
                  }
                  else
                  {
      ...
      function hire_job_get($job_id='',$job_name='',$employer_id='',$employee_id='', $user_type = '')
      {
            ...
            if(count($employee)){
                ...
                if($device == 'android'){
                    ...
                    $notification_data['message'] = $message;
                    $notification_data['count']   = $notification_count;
                    $notification_data['user_id'] = $employee_id;
                    $notification_data['type']    = "hirejob";
                send_android_notification($employee['devicetoken'],$notification_data);
                }
                else
                {
           ...
           // $this->db->query("delete from applied_jobs where job_id='".$job_id."' and employer_id='".$employer_id."' and  employee_id='".$employee_id."'");
            ...
            if(count($employee)) {
                ...
                if($device == 'android') {
                    ...
                    $notification_data['count']   = $notification_count;
                    $notification_data['user_id'] = $employee_id;
                    $notification_data['type']    = "refusejob";
                    send_android_notification($employee['devicetoken'],$notification_data);
                }
                else
                {
           ...
           // $message  = "Payment is made for ".rtrim(ucfirst($job_name),"").", provide star ratings";
            ...
            if(count($employee)){
             ...
             //   sendNotification($employer['devicetoken'],$message,count($params),$employer_id,'paymentcompleted');
                ...
                if($device == 'android') {
                    $notification_data = array();
                    $notification_data['message'] = $employee_msg;
                    $notification_data['count']   = $employee_notification_count;
                    $notification_data['user_id'] = $employee_id;
                    $notification_data['type']    = "notificationCountJobHistory";
                    send_android_notification($employee['devicetoken'],$notification_data);

                    $notification_data = array();
                    $notification_data['message'] = $employer_msg;
                    $notification_data['count']   = $employer_notification_count;
                    $notification_data['user_id'] = $employer_id;
                    $notification_data['type']    = "notificationCountJobHistory";
                    send_android_notification($employer['devicetoken'],$notification_data);
                }
                else
                {
           ...
           // $employer = $this->user_model->check_unique(array("id" => $employer_id));
            ...
            // {
                ...
                if($device == 'android') {
                    ...
                    $notification_data['type']    = "starNotification";
                    send_android_notification($userdata['devicetoken'],$notification_data);
                }
                else
                {
        ...
        function send_message_post()
        {
            ...
            if(count($userdata)) {
               ...
               if($device == 'android') {
                    ...
                    $notification_data['count']   = $notification_count;
                    $notification_data['user_id'] = $user_id;
                    $notification_data['type']    = "notificationCountMessage";
                    send_android_notification($userdata['devicetoken'],$notification_data);
                }
                else
                { 
        ...
        function employee_accept_post()
        {
            ...
            if(count($userdata)){
                ...
                if($device == 'android') {
                    ...
                    $notification_data['count']   = $notification_count;
                    $notification_data['user_id'] = $employer_id;
                    $notification_data['type']    = "notificationCountActive";
                    send_android_notification($userdata['devicetoken'],$notification_data);
                }
                else
                {
        ...
        function request_payment_notification_post()
        {
            ...
            if(count($userdata)){
                ...
                if($device == 'android') {
                    ...
                    $notification_data['count']   = $notification_count;
                    $notification_data['user_id'] = $employer_id;
                    $notification_data['type']    = "notificationCountMakePayment";
                    send_android_notification($userdata['devicetoken'],$notification_data);
                }
                else
                {
            ...
            $channel_id  = $employer_id.$employee_id;
             ...
             if(count($employerdata) && count($employeedata)){
                ...
                if($device == 'android') {
                    ...
                    $notification_data['count']   = $employer_notification_count;
                    $notification_data['user_id'] = $employer_id;
                    $notification_data['type']    = "jobcanceled";
                    send_android_notification($employerdata['devicetoken'],$notification_data);
                    ...
                    $notification_data['user_id'] = $employee_id;
                    $notification_data['type']    = "jobcanceled";
                    send_android_notification($employeedata['devicetoken'],$notification_data);
