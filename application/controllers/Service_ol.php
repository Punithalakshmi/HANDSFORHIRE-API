...
//             }
       ...
       //update profile image & profile name  
...

        {
            ...
            $job_expire_date_time   = $this->post('job_expire_date_time');
            ...
            $ins_data['delist']                = 'no';
            $ins_data['job_expire_date_time']  = $job_expire_date_time;
...
//          }
        ...
        function job_search_post()
        {
            ...
            if(count($job_lists)>0){
                 return $this->response(array('status' =>'success','request_type' => 'job_lists', 'job_lists' => $job_lists,"all_jobs" => $all_jobs, "query" => $query), 200);
            }
            else
            {
                 return $this->response(array('status' =>'error','request_type' => 'job_lists', 'msg' => "No Jobs Found" ,'error_code' => "8"), 404);
       ...
       //refuse active jobs
        ...
        function payment_service_post()
        {
            ...
            if(count($employee)){
                ...
                $employee_msg = ucfirst($job_name).' job has been completed. Fill out a star rating of your employee under job history. ';
                 $employer_msg = ucfirst($job_name).' job has been completed. Fill out a star rating of your employer under job history. ';
                sendNotification($employee['devicetoken'],$employee_msg,count($params),$employee_id,'notificationCountJobHistory');

                sendNotification($employer['devicetoken'],$employer_msg,count($params),$employer_id,'notificationCountJobHistory');

            }    
           update_notification_count($employee_id,$message,'paymentcompleted',$job_id,$user_type);
           update_notification_count($employer_id,$message,'paymentcompleted',$job_id,$user_type); 
       ...
       function review_rating_post()
        {
            ...
            {
                return $this->response(array('status' =>'error','request_type' => 'review_rating_lists', 'msg' => "This User Currently Does Not Have Any Ratings" ,'error_code' => "8"), 404);
            }     
        }
            ...
            $channel_id = $employer_id.$employee_id;
             ...
             if(count($employerdata) && count($employeedata)){
               ...
               sendNotification($employeedata['devicetoken'],$message,count($employee_params),$employee_id,'jobcanceled');
                ...
                $employee_msg = ucfirst($job['job_name']).' Job Has Been Canceled. Fill Out A Comments Of Your Employee Under Job History. ';
                $employer_msg = ucfirst($job['job_name']).' Job Has Been Canceled. Fill Out A Comments Of Your Employer Under Job History. ';
